angular.module("sltcApp").controller("dcTransdisciplinaryPlanCtrl", ['$rootScope', '$scope', 'DCProfessionalteamRes', 'DCProfessionalteamHisRes', 'DCProfessionalteamExtRes', 'empFileRes', 'utility', '$state', '$filter', function ($rootScope, $scope, DCProfessionalteamRes, DCProfessionalteamHisRes, DCProfessionalteamExtRes, empFileRes, utility, $state, $filter) {
    ...
    $scope.init = function () {
        ...
        empFileRes.get({ empNo: '', empName: '', empGroup: '009', currentPage: 0, pageSize: 0 }, function (response) {       
            $scope.NurseAides = response.Data;
        });
    ...
    $scope.PrintPreview = function () {
        if ($scope.Data!=null&&angular.isDefined($scope.Data.SEQNO) && $scope.Data.SEQNO != null && $scope.Data.SEQNO > 0) {
            window.open('/DC_Report/PreviewNurseCare?templateName=DCC3.0&seqNo=' + $scope.Data.SEQNO);
            }else {
    ...
    $scope.DownloadWord = function () {
        if ($scope.Data != null && angular.isDefined($scope.Data.SEQNO) && $scope.Data.SEQNO != null && $scope.Data.SEQNO > 0) {
            window.open('/DC_Report/DownLoadCrossReport?templateName=DCC3.0&seqNo=' + $scope.Data.SEQNO);
        } else {
