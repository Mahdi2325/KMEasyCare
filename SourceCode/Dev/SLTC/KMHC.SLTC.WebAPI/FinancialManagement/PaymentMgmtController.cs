...
namespace KMHC.SLTC.WebAPI
{
    ...
    public class PaymentMgmtController : BaseController
    {
        IPaymentMgmtService service = IOCContainer.Instance.Resolve<IPaymentMgmtService>();
        [Route(""), HttpGet]
        ...
        [Route(""), HttpGet]
        public IHttpActionResult GetNCIFinancialMonth(string FMonth)
        {
            var response = service.GetNCIFinancialMonth(FMonth);
            return Ok(response);
        }
        ...
        /// <returns></returns>
        [Route(""), HttpGet]
        public IHttpActionResult GetRsChargeGroup(int FeeNo, int FeeRecordId, int ChargeRecordType)
        {
            BaseRequest<PaymentMgmtFilter> request = new BaseRequest<PaymentMgmtFilter>
            {
                Data = { FEENO = FeeNo, FEERECORDID = FeeRecordId, CHARGERECORDTYPE = ChargeRecordType }
            };
