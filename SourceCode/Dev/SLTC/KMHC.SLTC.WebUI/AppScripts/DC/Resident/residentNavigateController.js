...
.controller("residentNavigateCtrl", ['$scope', '$state', '$compile', 'utility', 'dc_AssignJobsRes', 'dc_AssignTaskRes', '$filter', function ($scope, $state,$compile, utility, dc_AssignJobsRes, dc_AssignTaskRes, $filter) {
    $scope.Data = {};
    $scope.LastData = {};
    $scope.filter = { RecStatus: null, NewRecFlag: null, AssignStartDate: null, AssignEndDate: null };
    $scope.init = function () {
        $scope.options = {
            buttons: [],//需要打印按钮时设置
    ...
    $scope.showCalendar = function () {
        var html = '<div km-include km-template="Views/DC/Resident/workCalendar.html" km-controller="workCalendarCtrl"></div>';
        $scope.dialog = BootstrapDialog.show({
            title: '<label class=" control-label">工作日历</label>',
    ...
    $scope.EmpGroup = {};
    $scope.init = function () {
        dc_AssignTaskRes.get({}, function (req) {
            $scope.Data = req.Data;
