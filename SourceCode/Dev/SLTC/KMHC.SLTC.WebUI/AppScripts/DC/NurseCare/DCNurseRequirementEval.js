/*****************************************************************************
 * Filename: DCNurseRequirementEval
 * Creator: 张祥
 * Create Date: 2016-08-16 19:34:19
 * Description: 日照护理评估
 * Modifier:
 * Modify Date: 2016-08-20 19:34:19
 * Modify Description: 各评估表单数据获取ID写死了,修改为根据CODE动态获取ID
 ******************************************************************************/
angular.module("sltcApp").controller("DCNurseRequirementEvalCtrl", ['$rootScope', '$scope', '$filter', 'DCNurseRequirementEvalRes', 'DCevalsheetRes', 'DCNursePlanRes2', 'DCNursePlanRes', 'DCNsCplGoalRes', 'DCNsCplActivityRes', 'DCAssessValueRes', 'utility', '$state', function ($rootScope, $scope, $filter, DCNurseRequirementEvalRes, DCevalsheetRes, DCNursePlanRes2, DCNursePlanRes, DCNsCplGoalRes, DCNsCplActivityRes, DCAssessValueRes, utility, $state) {
    ...
    $scope.PrintPreview = function () {
        if (angular.isDefined($scope.currentItem.Id)) {
            window.open('/DC_Report/PreviewNursingReport?templateName=DCN1.2&id=' + $scope.currentItem.Id);
        } else {
    ...
    $scope.DownloadWord = function () {
        if (angular.isDefined($scope.currentItem.Id)) {
            window.open('/DC_Report/DownLoadReport?templateName=DCN1.2&id=' + $scope.currentItem.Id);
        } else {
    ...
    $scope.loadAdlEditInfo = function (item, recId) {
        ...
        DCevalsheetRes.get({ questionCode: questionCode, regNo: item.RegNo, recordId: recId }, function (response) {
    ...
    $scope.changeDiaPr = function (val) {
        ...
        DCNursePlanRes.get({ cpNo: val, dirPr: "", mark: "CpReason" }, function (data) {
            $scope.Data.causepList = {};
            $scope.Data.causepList = data.Data;
            $scope.nursingPlanItem.NsDesc = "";
        });

        DCNursePlanRes.get({ cpNo: val, dirPr: "", mark: "NsDesc" }, function (data) {
            $scope.Data.prDataList = {};
    ...
    $scope.editNursingPlanGoal = function (item) {
        $scope.Data.nsCplGoalpList = [];
        DCNursePlanRes.get({ cpNo: 0, dirPr: item.CpDia, mark: "Goal" }, function (data) {
            $scope.Data.goalpList = {};
    ...
    $scope.editNursingPlanActivity = function (item) {
        $scope.Data.nsCplActivityList = [];
        DCNursePlanRes.get({ cpNo: 0, dirPr: item.CpDia, mark: "Activity" }, function (data) {
            $scope.Data.activityList = {};
    ...
    $scope.editNursingPlanEval = function (item) {
        DCNursePlanRes.get({ cpNo: 0, dirPr: item.CpDia, mark: "Assessvalue" }, function (data) {
            $scope.assessItem = {};
    ...
    $scope.staffSelected = function (item) {
        $scope.assessItem.ExecuteBy = item.EmpNo;
    }

    $scope.staffRecordSelected = function (item) {
        $scope.assessItem.RecordBy = item.EmpNo;
    }
    $scope.init();
