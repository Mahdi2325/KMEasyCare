var App = function () {
	...
	var handleThemeSkins = function () {
        ...
        var setSkin = function (color) {
            $('#skin-switcher').attr("href", "/Content/CloudAdmin/css/themes/" + color + ".css");
		...
		if ($.cookie('gritter_show')) {
            ...
            setTimeout(function () {
                var unique_id = $.gritter.add({
                    ...
                    image: '../Content/CloudAdmin/img/gritter/cloud.png',
            ...
            setTimeout(function () {
                var unique_id = $.gritter.add({
                    ...
                    // (string | optional) the image to display on the left
                    image: '../Content/CloudAdmin/img/gritter/settings.png',
                    // (bool | optional) if you want it to fade out on its own or just sit there
                    sticky: true,
                    // (int | optional) the time you want it to be alive for before fading out
                    time: '',
            ...
            setTimeout(function () {
                ...
                var unique_id = $.gritter.add({
                    ...
                    // (string | optional) the image to display on the left
                    image: '../Content/CloudAdmin/img/gritter/buy.png',
                    // (bool | optional) if you want it to fade out on its own or just sit there
                    sticky: true,
                    // (int | optional) the time you want it to be alive for before fading out
                    time: '',
	...
	var getMenus=function(menus) {
	    if (menus == null) return "";
	    var strMenu = "";
	    for (var i = 0; i < menus.length; i++) {
	        if (i === 0) {
	            strMenu += '<li class="has-sub open"><a href="javascript:;" class=""><i class="fa fa-bookmark-o fa-fw"></i> <span class="menu-text">' + menus[i].name + '</span><span class="arrow open"></span></a>';
	        } else {
	            strMenu += '<li class="has-sub"><a><i class="fa fa-bookmark-o fa-fw"></i><span class="sub-menu-text">' + menus[i].name + '</span><span class="arrow"></span></a>';
	        }
	        if (menus[i].nodes == null) {
	            strMenu += "</li>";
	            continue;
	        }
	        if (i === 0) {
	            strMenu += '<ul class="sub" style="display: block">';
	        } else {
	            strMenu += '<ul class="sub">';
	        }
	        for (var j = 0; j < menus[i].nodes.length; j++) {
	            if (i === 0 && j === 0) {
	                strMenu += '<li class="active"><a class="" href="#" url="' + menus[i].nodes[j].url + '" status="' + menus[i].nodes[j].status + '"><span class="sub-menu-text">' + menus[i].nodes[j].name + '</span></a></li>';
	                //strMenu += '<li class="active"><a class="" href="' + menus[i].nodes[j].url + '"><span class="sub-menu-text">' + menus[i].nodes[j].name + '</span></a></li>';
	            } else {
	                strMenu += '<li><a class="" href="#" url="' + menus[i].nodes[j].url + '" status="' + menus[i].nodes[j].status + '"><span class="sub-menu-text">' + menus[i].nodes[j].name + '</span></a></li>';
	                //strMenu += '<li><a class="" href="' + menus[i].nodes[j].url + '"><span class="sub-menu-text">' + menus[i].nodes[j].name + '</span></a></li>';
	            }
	        }
	        strMenu += "</ul></li>";
	    }
	...
	return {
	    initMenu: function (roleId) {
	        $.ajax({
	            type: "GET",
	            url: "/api/module",
	            data: {roleId:roleId},
	            success: function (data) {
	                $("#ulMenu").append(getMenus(data));
	    ...
	    init: function () {
	        //$('#selectItems').on('show.bs.modal', function (e) {
