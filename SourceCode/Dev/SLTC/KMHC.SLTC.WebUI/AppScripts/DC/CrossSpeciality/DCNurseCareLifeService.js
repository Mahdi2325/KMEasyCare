angular.module("sltcApp").controller("NurseCareLifeServiceCtrl", ['$rootScope', '$scope', 'dictionary', 'DCNurseCareLifeServiceRes', 'DCNurseCareLifeList', 'DCNurseCareLifeEdit', 'utility', '$state', 'empFileRes', function ($rootScope, $scope, dictionary, DCNurseCareLifeServiceRes, DCNurseCareLifeList, DCNurseCareLifeEdit, utility, $state, empFileRes) {
    ...
    $scope.fristquery = function () {
        DCNurseCareLifeServiceRes.get({}, function (obj) {
            //求现在的周数
    ...
    $scope.PrintPreview = function () {
        if (angular.isDefined($scope.NURSE.ID) && $scope.NURSE.ID != 0) {
            ...
            window.open('/DC_Report/PreviewNurseCare?templateName=DCC1.1&recId=' + $scope.NURSE.ID);
    ...
    $scope.DownloadWord = function () {
        if (angular.isDefined($scope.NURSE.ID) && $scope.NURSE.ID != 0) {
            ...
            window.open('/DC_Report/DownLoadCrossReport?templateName=DCC1.1&recId=' + $scope.NURSE.ID);

        } else {
            utility.message("未获取到护理及生活照顾服务记录表,无法导出！");
        }
    }
