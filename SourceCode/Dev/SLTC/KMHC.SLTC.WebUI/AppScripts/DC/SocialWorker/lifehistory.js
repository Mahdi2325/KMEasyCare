...
.controller('lifehistoryCtrl', ['$rootScope', '$scope', 'utility', '$state', 'dictionary', 'dc_PersonBasicRes', 'dc_LifeHistoryRes',
    function ($rootScope, $scope, utility, $state, dictionary, dc_PersonBasicRes, dc_LifeHistoryRes) {
    ...
    $scope.PrintPreview = function () {
        ...
        if (angular.isDefined($scope.currentItem.Id)) {
            window.open('/DC_Report/Preview?templateName=DCS1.1&recId=' + $scope.currentItem.Id);
        } else {
    ...
    $scope.DownloadWord = function () {
        ...
        if (angular.isDefined($scope.currentItem.Id)) {
            window.open('/DC_Report/DownLoadSocialReport?templateName=DCS1.1&recId=' + $scope.currentItem.Id);
        } else {
        ...
        if (angular.isDefined(error)) {
            ...
            if (error.minlength) {
                $.each(error.minlength, function (n, value) {
                    $scope.errArr.push(value.$name + "超过设定最小录入长度！");
                });
            ...
            if (error.maxlength) {
                $.each(error.maxlength, function (n, value) {
                    ...
                    $scope.errArr.push(value.$name + "超过设定最大录入长度！");
                });
            }
            //return errorMsg;
        }
