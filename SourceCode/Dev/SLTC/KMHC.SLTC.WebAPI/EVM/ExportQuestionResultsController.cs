...
{
    [RoutePrefix("api/exportQueRes")]
    public class ExportQuestionResultsController : BaseController
    {
        ...
        public IHttpActionResult Post(LTC_Question request)
        {
            ...
            var response = Service.SaveResultModleData(request.QuestionId??0, request.OrgId,request.ExportQuestionId??0);
            return Ok(response);
        }
