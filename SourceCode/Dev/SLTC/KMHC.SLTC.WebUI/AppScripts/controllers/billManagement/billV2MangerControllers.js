...
.controller("BillV2MangerCtrl", ['$scope', '$filter', '$stateParams', 'billV2Res', 'utility', '$state', function ($scope, $filter, $stateParams, billV2Res, utility, $state) {
    ...
    $scope.deleteItem = function (item) {
        if (item.Status == 0) {
            if (confirm("确定删除该条账单信息吗?")) {
                billV2Res.get({ id: item.BillId }, function (data) {
                    $scope.options.pageInfo.CurrentPage = 1;
                    $scope.options.search();
            ...
            utility.msgwarning("当前账单处于已缴费或已退款状态！");
        }
    };
