...
.controller('swregevalplanCtrl', ['$rootScope', '$scope', 'utility', '$state', 'dictionary', 'cloudAdminUi', 'dc_PersonBasicRes', 'dc_SwRegEvalPlan', 'DCNursePlanRes', 'dc_TaskgoalsstrategyRes',
    function ($rootScope,$scope, utility, $state, dictionary, cloudAdminUi, dc_PersonBasicRes, dc_SwRegEvalPlan, DCNursePlanRes, dc_TaskgoalsstrategyRes) {
   ...
   　//控制当前页面功能按纽可见
    ...
    $scope.changLevel = function (item) {
        ...
        DCNursePlanRes.get({ cpNo: item.CpNo, dirPr: '', mark: "CpReason" }, function (data) {
    ...
    $scope.PrintPreview = function (id) {
        if (angular.isDefined(id) && id!=null && id>0) {
            window.open('/DC_Report/Preview?templateName=DCS1.9&recId=' + id + "&feeNo="+$scope.FeeNo);
        } else {
    ...
    $scope.DownloadWord = function (id) {
        if (angular.isDefined(id) && id != null && id > 0) {
            window.open('/DC_Report/DownLoadSocialReport?templateName=DCS1.9&recId=' + id + "&feeNo=" + $scope.FeeNo);
        } else {
            utility.message("未获取到个案评估记录,无法导出！");
        }
