...
.controller('personbasicCtrl', ['$rootScope', '$scope', '$state', 'utility', 'dictionary', 'webUploader', 'dc_PersonBasicRes', function ($rootScope,$scope, $state, utility, dictionary, webUploader, dc_PersonBasicRes) {
            ...
            if (error.minlength) {
                $.each(error.minlength, function (n, value) {
                    if (value.$name == "身份证号")
                        ...
                        $scope.errArr.push(value.$name + "录入字符长度超过规定最小值");
                });
            ...
            if (error.maxlength) {
                $.each(error.maxlength, function (n, value) {
                    if (value.$name == "身份证号")
                        ...
                        $scope.errArr.push(value.$name + "录入字符长度超过规定最大值");
                });
            }debugger
            if (error.pattern) {
    ...
    $scope.PrintPreview = function () {
        ...
        if (angular.isDefined($scope.FeeNo)) {
            window.open('/DC_Report/Preview?templateName=DCS1.3&recId=' + $scope.FeeNo);
        } else {
    ...
    $scope.DownloadWord = function () {
        ...
        if (angular.isDefined($scope.FeeNo)) {
            window.open('/DC_Report/DownLoadSocialReport?templateName=DCS1.3&recId=' + $scope.FeeNo);
        } else {
