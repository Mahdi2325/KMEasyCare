angular.module("sltcApp").controller("DCregMedcineCtrl", ['$rootScope', '$scope', 'DrugManageRes', 'utility', '$state', function ($rootScope, $scope, DrugManageRes, utility, $state) {
    ...
    $scope.PrintPreview = function () {
        if (angular.isDefined($scope.currentResident.FeeNo)) {
            ...
            window.open('/DC_Report/PreviewNursingReport?templateName=DCN1.1&feeNo=' + $scope.currentResident.FeeNo);
        } else {
            utility.message("无打印数据");
        }
    }
    $scope.DownloadWord = function () {
        if (angular.isDefined($scope.currentResident.FeeNo)) {
            ...
            window.open('/DC_Report/DownLoadReport?templateName=DCN1.1&feeNo=' + $scope.currentResident.FeeNo);
        } else {
            utility.message("无导出数据");
        }
    }
