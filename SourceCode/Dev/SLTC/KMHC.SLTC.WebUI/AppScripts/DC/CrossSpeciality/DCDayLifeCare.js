angular.module("sltcApp").controller("DCDayLifeCareCtrl", ['$rootScope', '$scope', 'dictionary', 'DCDayLifeCareRes', 'DCDayLifeCareList', 'DCDayLifeCareListA', 'utility', '$state', 'empFileRes', function ($rootScope,$scope, dictionary, DCDayLifeCareRes, DCDayLifeCareList, DCDayLifeCareListA, utility, $state, empFileRes) {
    ...
    {
        DCDayLifeCareRes.get({}, function (obj) {
            //求现在的周数
   ...
   // 查询的东西
    ...
    $scope.saveForm = function (item1, item2, item3, item4, item5, DAYLIFE) {
        ...
        var DayLifeRec = {
            ...
            ORGID: $scope.currentResident.OrgId,
            FEENO: $scope.FeeNo,
    ...
    $scope.PrintPreview = function () {
        if (angular.isDefined($scope.DAYLIFE.ID) && $scope.DAYLIFE.ID!=0) {
            ...
            window.open('/DC_Report/PreviewNurseCare?templateName=DCC1.2&recId=' + $scope.DAYLIFE.ID);
        } else {
    ...
    $scope.DownloadWord = function () {
        if (angular.isDefined($scope.DAYLIFE.ID) && $scope.DAYLIFE.ID != 0) {
            ...
            window.open('/DC_Report/DownLoadCrossReport?templateName=DCC1.2&recId=' + $scope.DAYLIFE.ID);
        } else {
            utility.message("未获取到日常生活照顾记录表,无法导出！");
        }
    }
