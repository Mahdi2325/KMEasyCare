angular.module("sltcApp")
    .controller("msgListCtr", ['$rootScope', '$scope', '$filter', '$compile', '$state', 'adminHandoversRes', 'dc_AssignJobsRes', '$cookieStore', function ($rootScope, $scope, $filter, $compile, $state, adminHandoversRes, dc_AssignJobsRes, $cookieStore) {
        $scope.init = function() {
            $scope.Data = {
                Msgs: []
            }
            ...
            if (isEmpty(global) || global.CurrentLoginSys!='LC') {
...
.controller("menuCtr", ['$scope', '$state', '$location', 'cloudAdminUi', 'roleModuleRes', function ($scope, $state, $location, cloudAdminUi, roleModuleRes) {
        $scope.init = function () {
            $scope.menus = [];
        ...
        $scope.subMenuClick = function ($event, url, way)
        {
           ...
           // jQuery(target).parent().parent().children(".active").removeClass("active");
            ...
            if (way==='1') {
                ...
                if (id != '' && (url.length - 1) == url.lastIndexOf('/')) {
                    url = url + id;
                }
                $location.url(url);
            } else {
