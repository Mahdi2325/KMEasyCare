...
(function () {
    ...
    app.factory("utility", ['$cookies', '$rootScope', 'dictManageRes', function ($cookies, $rootScope, dictManageRes) {
        return {
            ...
            getFeeIntervalByMonth: function (month) {
                return dictManageRes.GetFeeIntervalByMonth({ month: month });
            },
            getFeeIntervalByDateStr: function (date) {
                return dictManageRes.GetFeeIntervalByDateStr({ date: date });
            },
            getFeeIntervalByDateTime: function (date) {
                return dictManageRes.GetFeeIntervalByDateTime({ date: date });
            },
            getFeeIntervalByYearMonth: function (yearMonth) {
                return dictManageRes.GetFeeIntervalByYearMonth({ yearMonth: yearMonth });
            },
            ...
            Validation: function (error) {
                var errorTips = 0;
                var maxErrorTips = 3;
                if (angular.isDefined(error.required)) {
                    var msg = "";
                    for (var i = 0; i < error.required.length; i++) {
                        msg = error.required[i].$name + " 为必填项";
                        this.msgwarning(msg);
                        errorTips++;
                        if (errorTips >= maxErrorTips) {
                            return false;
                        }
                    }
                }

                if (angular.isDefined(error.maxlength)) {
                    var msg = "";
                    for (var i = 0; i < error.maxlength.length; i++) {
                        msg = error.maxlength[i].$name + "超过设定长度 ";
                        this.msgwarning(msg);
                        errorTips++;
                        if (errorTips >= maxErrorTips) {
                            return false;
                ...
                if (angular.isDefined(error.number)) {
                    var msg = "";
                    for (var i = 0; i < error.number.length; i++) {
                        msg = error.number[i].$name + "必须为数字 ";
                        this.msgwarning(msg);
                        errorTips++;
                        if (errorTips >= maxErrorTips) {
                            return false;
                ...
                if (angular.isDefined(error.pattern)) {
                    var msg = "";
                    for (var i = 0; i < error.pattern.length; i++) {
                        if (!isEmpty(getValidateMsg(error.pattern[i].$name))) {
                            msg = error.pattern[i].$name + getValidateMsg(error.pattern[i].$name);
                            this.msgwarning(msg);
                            errorTips++;
                            if (errorTips >= maxErrorTips) {
                                return false;
